...
export default {
  ...
  mounted() {
    this.isLoading = true;
    PurchaseService.getLastPurchaseId().then((item) => {
      const purchaseId = item.data.data.length !== 0 ? item.data.data.id : 0;
      this.purchase.supplier_invoice_number = String(purchaseId + 1).padStart(
        6,
        "0"
      );
  ...
  methods: {
    ...
    createNewPurchase() {
      ...
      if (this.purchase) {
        PurchaseService.postPurchase(this.purchase).then(
          () => {
