<template>
  ...
  <template v-if="!isLoading && receipt && customerInfo">
    <div class="row mb-3">
    ...
    <div class="row justify-content-between mb-3">
      ...
      <div class="col-lg-5 text-right">
        <button
          @click.prevent="prepareReceiptObject(true)"
          :disabled="invalid"
          type="button"
        ...
        <button
          @click.prevent="prepareReceiptObject()"
          :disabled="invalid"
          type="button"
          class="btn btn-default"
        >
...
export default {
  ...
  mounted() {
    ...
    ReceiptService.getReceiptById(this.receiptId).then((item) => {
      ...
      this.isGeneralCustomer = receipt.customer_id === null;
      CustomerService.getCustomers({ limit: 0 }).then((customers) => {
        ...
        if (!this.isGeneralCustomer) {
          this.onChangeCustomer(receipt.customer_id);
        } else {
      ...
      receipt.signature = receipt.signature ? true : false;
      this.checkTransferType(receipt.type, true);
  ...
  methods: {
    ...
    updateReceipt(receipt, isPrint) {
      ReceiptService.updateReceipt(this.receiptId, receipt).then(
        (result) => {
